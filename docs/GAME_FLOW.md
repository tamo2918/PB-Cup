...
ゲージ表示と同時に**全チームの回答マーカー**（チーム色の縦線＋ラベル）が一斉に並び、その後ゆっくりとポインタが走る — 「どのチームの近くで止まるか」を視聴者が固唾を飲んで見守る、焦らしの演出。

```
T=0       ゲージバー＋全チームマーカー(色付き縦線)が同時に登場
T=0–0.7s  チームマーカーが stagger fade-in
Phase A:  ポインタ(金色) 0% ──slow slide──► overshoot (target × 1.8 〜 +25 のうち大)
...
Phase C:  "ドン" 効果音と同時に
            ・ポインタが金色 → 真っ赤＋発光 に変化（赤い線が確定）
            ・ポインタの三角と縦線が拡大 (scale 1 → 1.5 → 1.15)
            ・正解の % が 0.5 → 1.5 → 1.0 でドン！と表示
            ・ゴールド fill バー (0% → correctAnswer%) がアニメで追従
```

設計上のポイント:

- アニメ中のポインタは**金色**で、視聴者の注目を引きつつまだ「答え」とは確定していない雰囲気
- 収束した瞬間に**赤いネオン光のような発光**を伴う太い赤線へ変化 → 「これが答えだ！」と一目で分かる演出
- ライブで動くポインタの位置は `data-gauge-pointer` 属性付きの DOM から `getComputedStyle` で `left` を取り出して計算（Framer Motion の値読み取りに頼らずシンプルに）。
